...
class HttpBotDeliveryGatewayTest < Minitest::Test
  ...
  def test_delivers_each_intent_chunk_to_the_bound_telegram_surface
    ...
    assert_equal ["digest-1", "digest-1"], transport.requests.map { |request| JSON.parse(request.fetch(:body)).fetch("message_thread_id") }
    assert_equal ["secret"], transport.requests.map { |request| request.fetch(:headers).fetch("x-prism-bot-delivery-secret") }.uniq
    assert transport.requests.map { |request| JSON.parse(request.fetch(:body)).fetch("idempotency_key") }.all? { |key| key.match?(/\A[0-9a-f]{64}\z/) }
    refute_equal JSON.parse(transport.requests[0].fetch(:body)).fetch("idempotency_key"), JSON.parse(transport.requests[1].fetch(:body)).fetch("idempotency_key")
  end

  def test_rejects_a_binding_for_another_logical_context
    error = assert_raises(PrismHub::InputError) do
      gateway_for(FakeTransport.new([])).deliver(
        intent: intent_for(["first"]),
        binding: binding_for(logical_channel: "other")
      )
    end
  ...
  def response_for(message_id)
    {
      ...
      body: JSON.generate(
        "status" => "ok",
        "delivery" => {"provider_message_id" => message_id, "idempotency_key" => nil}
      )
    }
  end
  ...
  def intent_for(texts)
    ...
    fingerprint = Digest::SHA256.hexdigest([
      value.fetch("artifact_kind"), value.fetch("artifact_id"), value.fetch("workspace"),
      value.fetch("channel"), value.fetch("format"), texts.join
    ].join("\0"))
    PrismHub::Domain::DeliveryIntent.new(
      artifact_id: value.fetch("artifact_id"), artifact_kind: value.fetch("artifact_kind"),
      workspace: value.fetch("workspace"), channel: value.fetch("channel"), format: value.fetch("format"),
      chunks: chunks, idempotency_key: fingerprint
    )
  end

  def binding_for(logical_channel: "digest")
    PrismHub::Domain::TelegramSurfaceBinding.new(
      id: "binding-1", workspace_id: "workspace-1", bot_instance_id: "bot-1",
      logical_channel: logical_channel, chat_id: "chat-123", message_thread_id: 42,
