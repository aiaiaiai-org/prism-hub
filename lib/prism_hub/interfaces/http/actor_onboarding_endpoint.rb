...
module PrismHub
  module Interfaces
    module Http
      class ActorOnboardingEndpoint
        REQUIRED_FIELDS = %w[provider provider_scope subject_id].freeze

        def initialize(onboard_provider_identity:, request_body:)
          @onboard_provider_identity = onboard_provider_identity
          @request_body = request_body
        end

        def call(request, authorisation_context:)
          subject = provider_subject(@request_body.parse(request))
          context = @onboard_provider_identity.call(
        ...
        def provider_subject(payload)
          ...
          raise InputError.new(
            ...
            "onboarding request must contain only provider, provider_scope, and subject_id strings"
          )
        end
