  ...
  module UseCases
    class GenerateMailDigest
      ...
      def call(mailbox_id:, since:, before:)
        ...
        credential = refresh_if_needed(credential)
      ...
      def validate_credential!(credential)
        ...
        return if credential.token_type.casecmp?("Bearer")
      ...
      def validate_refreshed_token!(result)
        unless result.scope.include?(READ_SCOPE) && result.token_type.casecmp?("Bearer")
          raise AuthorisationError.new(
