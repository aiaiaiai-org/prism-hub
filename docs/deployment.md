...
- passwordless `sudo` for the service-management operations used by `script/deploy-production`;
...
In GitHub, open **Actions → Deploy production → Run workflow** and select `master`. GitHub applies the `production` Environment controls before the job can access its Environment secrets.

The workflow refuses to activate any non-`master` ref. After local activation succeeds, it performs the optional public health check when `PRISM_HUB_PUBLIC_URL` is configured.
