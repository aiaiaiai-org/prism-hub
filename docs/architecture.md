...
| Use cases | Channel discovery, execution orchestration, and client-credential lifecycle | ActiveRecord, process spawning, provider tokens, HTTP responses |
| Ports | Focused channel lookup, Prism execution, and client-credential persistence capabilities | Concrete storage or runtime choices |
...
Credential issue generates a high-entropy opaque `prism_client_v1_…` secret and
stores only its SHA-256 digest. Expiry and revocation are evaluated during
credential lookup. Rotation creates the replacement and revokes the prior
credential in one database transaction; revocation is idempotent.

The persistence adapter resolves a valid credential into an immutable
